...
phenotypeFactor.card = [phenotypes, numAlleles, numAlleles];
phenotypeFactor.val = zeros(1, prod(phenotypeFactor.card));
% Replace the zeros in phentoypeFactor.val with the correct values.
for m = 1:numAlleles
	for f = 1:numAlleles 
		val = alphaList(allelesToGenotypes(m, f));
%		phenotypeFactor = SetValueOfAssignment(phenotypeFactor, [1, m, f], val);
%		phenotypeFactor = SetValueOfAssignment(phenotypeFactor, [2, m, f], 1.0 - val);
		phenotypeFactor = SetValueOfAssignment(phenotypeFactor, [1, m, f; 2, m, f], [val, 1.0 - val]);
	end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

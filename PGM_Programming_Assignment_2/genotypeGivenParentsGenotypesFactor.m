...
k = length(genotypesToAlleles);
genotypeFactor.card = [k k k];

genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.
for i=1:k
	for j=1:k
		alleles1 = genotypesToAlleles(i, :);
		alleles2 = genotypesToAlleles(j, :);
		genotypes = zeros(1, k);
		++genotypes(allelesToGenotypes(alleles1(1), alleles2(1)));	
		++genotypes(allelesToGenotypes(alleles1(1), alleles2(2)));	
		++genotypes(allelesToGenotypes(alleles1(2), alleles2(1)));	
		++genotypes(allelesToGenotypes(alleles1(2), alleles2(2)));	
		genotypes /= sum(genotypes);
		for l=1:k
			genotypeFactor = SetValueOfAssignment(genotypeFactor, [l, i, j], genotypes(l));
		end
	end
end
